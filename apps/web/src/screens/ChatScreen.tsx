...
export function ChatScreen(props: { token: string; activeProfile: Profile | null; activeProject: Project | null; activeSession: ChatSession | null; profiles: Profile[]; projects: Project[]; onActiveProfile: (p: Profile) => void; onActiveProject: (p: Project | null) => void; onSession: (s: ChatSession) => void; onRefresh: () => Promise<void>; onNewSession: () => Promise<void> }) {
  ...
  return <section className="chat-stage"><div className="toolbar-row"><label className="toolbar-control" title="Project"><span className="toolbar-ico"><IconProjects size={15} /></span><select value={props.activeProject?.slug || ''} onChange={e => { if (!e.target.value) return props.onActiveProject(null); const p = props.projects.find(project => project.slug === e.target.value); if (p) props.onActiveProject(p) }}><option value="">No project</option>{props.projects.map(project => <option key={project.slug} value={project.slug}>{project.name}</option>)}</select></label><label className="toolbar-control" title="Agent profile"><span className="toolbar-ico"><IconAgents size={15} /></span><select value={props.activeProfile?.id || ''} onChange={e => { const p = props.profiles.find(profile => profile.id === Number(e.target.value)); if (p) props.onActiveProfile(p) }}>{props.profiles.map(profile => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label><span className={`stream-dot ${connected ? 'on' : ''}`} title={connected ? 'Stream connected' : 'Stream idle'} />{busyRun && <button className="ghost-button" onClick={() => void cancelRun(props.token, busyRun)} title="Cancel run">Cancel</button>}</div>{error && <div className="error-bar">{error}</div>}<ChatThread messages={messages} events={events} pendingRunId={busyRun} pendingText={busyRun ? 'Hermes is working…' : ''} /><Composer disabled={!props.activeProfile} token={props.token} onSubmit={submit} /></section>
}
